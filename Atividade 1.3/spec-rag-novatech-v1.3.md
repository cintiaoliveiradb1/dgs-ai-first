...
| Alterações v1.3 | AMB-01: "peso de recuperação mais alto" substituído por mecanismo concreto de re-ranking pós-recuperação com desempate por tipo de fonte, formalizado em F-006. AMB-02: janela de inconsistência entre ingestão primária (4h) e FAQ (24h) tratada em R-ATU-004b — FAQ conflitante é suprimido com sinalização ao atendente e marcado para revisão. AMB-03: R-NF-004 reescrito para distinguir explicitamente "gap documental" de "fora do escopo", com mensagens distintas e classificação baseada na lista de gaps da seção 2.3. |
...
Documentos normativos e contratuais com responsável formal declarado. Devem ser priorizados na composição das respostas por meio de re-ranking pós-recuperação: o pipeline busca os trechos mais relevantes de toda a base e, em caso de empate de score entre um trecho de fonte primária e um de fonte secundária, o trecho primário prevalece. Além disso, toda resposta baseada em fonte primária deve receber indicação de "fonte oficial" no resultado apresentado ao atendente.
...
| F-006 | **Re-ranking por tipo de fonte:** após a recuperação dos trechos mais relevantes, o pipeline deve aplicar uma etapa de re-ranking que, em caso de empate de score de similaridade entre um trecho de fonte primária (`tipo=normativo`, `procedimento` ou `contratual`) e um trecho de fonte secundária (`tipo=informal`), seleciona o trecho primário. A relevância semântica prevalece quando os scores forem claramente distintos; o tipo de fonte é o critério de desempate, não de substituição. | Teste com pergunta que retorna trecho primário e secundário com scores iguais: resposta usa o trecho primário. Teste com pergunta em que o trecho secundário tem score significativamente mais alto que o primário: resposta usa o trecho secundário (relevância prevalece). O mecanismo de re-ranking está documentado no relatório técnico de homologação. |
...
| R-ATU-004b | **Janela de inconsistência entre fontes primária e FAQ:** como documentos primários são ingeridos em até 4h e o FAQ em até 24h, existe uma janela em que ambos podem coexistir com conteúdos divergentes. Durante essa janela, quando o pipeline recuperar trechos de uma fonte primária e do FAQ sobre o mesmo tema com conteúdos conflitantes, o trecho do FAQ deve ser descartado da resposta e o atendente deve ser informado explicitamente: *"Esta resposta é baseada na documentação oficial. Uma versão anterior desta orientação existia no FAQ e foi desconsiderada por ser menos recente que o documento oficial."* O FAQ conflitante deve ser sinalizado internamente para revisão prioritária pela equipe responsável. | Teste com pergunta feita durante janela de inconsistência simulada (primária atualizada, FAQ ainda com versão antiga): resposta usa somente a fonte primária e exibe o aviso de descarte do FAQ. Painel de administração registra o item do FAQ como pendente de revisão. |
...
| R-NF-004 | **Escopo restrito e classificação de resposta negativa:** antes de responder que não possui informação, o assistente deve classificar a pergunta em uma de três categorias: (1) coberta pela base — responde normalmente; (2) tema logístico NovaTech sem cobertura documental (gap) — informa que o tema é operacionalmente relevante mas ainda sem documentação formal e orienta o atendente à área responsável; (3) fora do escopo do assistente — informa que o tema não pertence ao domínio de operações logísticas da NovaTech. A classificação de gap usa a lista de temas mapeados na seção 2.3 como referência. As mensagens das categorias 2 e 3 devem ser distintas e orientar o atendente de forma diferente. | Pergunta sobre seguro de carga (gap documentado): *"Esse tema faz parte das operações da NovaTech, mas ainda não há documentação formal disponível na base. Recomendo contatar a área de Comercial ou Compliance diretamente."* Pergunta sobre clima (fora do escopo): *"Esse tema está fora do escopo do assistente, que responde exclusivamente sobre operações logísticas da NovaTech."* Os dois cenários nunca exibem a mesma mensagem. |
...
*Versão 1.3 — Confidencial — NovaTech Logística*
