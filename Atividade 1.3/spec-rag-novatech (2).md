...
| ID | Requisito | Critério de Aceitação |
|----|-----------|----------------------|
| R-CONT-001 | O pipeline deve atribuir, no momento da indexação, os metadados `versão`, `data de emissão`, `status` (vigente / transição / obsoleto) e `tipo` (normativo / procedimento / contratual / informal) a cada documento. | Todos os documentos indexados contêm os quatro metadados obrigatórios. Indexação sem esses campos é rejeitada. |
...
> ℹ️ **Nota:** A seção 5 da PROC-042-v2 define uma data de corte (01/12/2023): chamados abertos antes dessa data devem usar a v1; os abertos depois, a v2. O pipeline deve suportar essa lógica condicional, idealmente perguntando ao atendente a data de abertura do chamado quando a informação não estiver no contexto da conversa.
...
| R-GAP-002 | O assistente não deve fabricar respostas. É proibido inferir, extrapolar ou gerar informações não presentes nos chunks recuperados. | Respostas com informações não contidas em nenhum chunk indexado devem ser consideradas falha grave em testes de aceitação. |
...
| R-RASTR-005 | O trecho exibido deve ser o chunk original recuperado, sem paráfrase, para permitir ao atendente verificar o contexto completo se necessário. | Atendente pode comparar a resposta com o trecho e confirmar que não houve distorção de sentido. |
...
| ID | Requisito | Critério de Aceitação |
|----|-----------|----------------------|
| R-RASTR-007 | Cada interação (pergunta + resposta) deve ser registrada em log com: timestamp, ID do atendente, pergunta, chunks recuperados (com score de similaridade), resposta gerada e fontes citadas. | Log auditável disponível por no mínimo 90 dias. Campo `chunks_recuperados` inclui `document_id`, `chunk_id` e `score`. |
| R-RASTR-008 | O log deve ser utilizável para auditoria retroativa em caso de reabertura de chamado, permitindo reconstruir qual resposta foi fornecida e com base em qual versão dos documentos. | Dado o ID de um chamado reaberto, é possível consultar o log e identificar exatamente qual chunk e versão de documento embasou a resposta original. |
