...
|---|---|---|---|---|
| **Frete especial** | Modalidade de transporte aplicável a cargas com peso **igual ou superior a 500 kg**, cujo valor é calculado pela fórmula: Valor base × Multiplicador regional × Fator de peso. Carga de exatamente 500 kg enquadra-se nesta modalidade com fator de peso 1,0. | "O chamado envolve frete especial pois a carga tem 800 kg." "Carga de 500 kg também é frete especial — fator de peso 1,0." | frete pesado, frete diferenciado, frete acima do padrão | PROC-042 v1 §1, PROC-042 v2 §1 — ver ADR-007 |
...
| **Devolução parcial** | Devolução de volumes individuais de uma entrega com múltiplos volumes. O reembolso é calculado na seguinte hierarquia: (1) proporcional ao **valor declarado por volume** no CT-e quando discriminado; (2) proporcional ao **peso por volume** no CT-e quando apenas peso for discriminado; (3) quando nenhum dos dois for discriminado por volume, o cálculo proporcional não pode ser feito automaticamente — encaminhar ao Operações para apuração manual. | "O cliente quer devolver 3 dos 10 volumes — verificar CT-e: se há valor por volume, usar valor; se só peso, usar peso; se não discriminado, escalar ao Operações." | devolução de parte, devolução incompleta | POL-001 v3.1 §3.4 — ver AMB-008 |
...
| **Incidente crítico** | Chamado que atende a pelo menos um de quatro critérios: (1) carga com valor declarado acima de R$ 100.000 com status desconhecido há mais de 6 horas; (2) carga perigosa com qualquer irregularidade de documentação ou rastreamento; (3) mais de 5 chamados do mesmo cliente nas últimas 24 horas sobre o **mesmo problema** — definido como chamados que compartilham o mesmo número de CT-e, **ou** a mesma categoria de chamado E o mesmo número de contrato, abertos nas últimas 24 horas; (4) risco à segurança de pessoas. O assistente classifica como incidente crítico com base na declaração do atendente, orientando-o a confirmar o número de chamados antes de escalar. | "Status desconhecido há 7h em carga de R$ 150k — classificar como incidente crítico." "Atendente relata 6 chamados do mesmo CT-e hoje — incidente crítico, confirmar e escalar." | urgente, crítico, prioritário, P1 | SLA-2024 §3 — ver ADR-008 |
...
| **Relógio de SLA** | Contagem de tempo que mede o cumprimento dos SLAs. Pausa fora do horário comercial para **chamados gerais** de qualquer tier. **Não pausa para incidentes críticos de nenhum tier** — o critério de incidente crítico é objetivo e independe do volume contratual do cliente. | "Chamado geral aberto às 17h30 — relógio de SLA pausa às 18h e retoma às 08h do próximo dia útil." "Incidente crítico de cliente Silver aberto às 22h — relógio contínuo, sem pausa." | contador de SLA, temporizador, cronômetro de atendimento | SLA-2024 §5 — ver ADR-008 |
...
| **Desconto por volume** | Redução percentual aplicada **sobre o multiplicador regional** quando o cliente ultrapassa limiares mensais de fretes especiais. A fórmula com desconto é: `Frete especial = Valor base × (Multiplicador regional × (1 − desconto%)) × Fator de peso`. **[CONFLITO]** v1 define limiar único de 10 fretes/mês; v2 define dois limiares: 8 fretes/mês (5% de desconto) e 15 fretes/mês (10% de desconto). FAQ-Item 45 alinha com v1, conflitando com v2. | "Cliente com 9 fretes especiais no mês, destino Norte: MR com desconto = 1,8 × 0,95 = 1,71. Frete = Valor base × 1,71 × Fator de peso." | desconto de fidelidade, desconto comercial, desconto de frequência | PROC-042 v1 §4, PROC-042 v2 §4, FAQ-Item 45 — ver ADR-004 |
...
| **Disposições transitórias** | Regras que definem qual versão do PROC-042 se aplica conforme a data de abertura do chamado: v1 para chamados abertos até 30/11/2023; v2 para chamados abertos a partir de 01/12/2023. **Fuso horário canônico: UTC-3 (America/Sao_Paulo).** Quando apenas a data for informada sem hora, tratar como 00h01 daquela data para fins de seleção de versão. | "Chamado aberto em 15/11/2023 — aplicar multiplicadores da v1 pelas disposições transitórias." "Data informada sem hora: tratar como 00h01 para garantir atribuição correta de versão." | regras de transição, período de transição | PROC-042 v2 §5 — ver ADR-003, RD-003-A |
